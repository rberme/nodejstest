...
import { AccessTokenData, ErrorData, MsgID, PaymentEvent, Products, TradeOrderID, UserInfoField } from './types';
...
export class GameServer {
    ...
    async SilentLogin(_code: string) {

    }

    async getUserInfo(_code: string, ...fields: UserInfoField[]) {
        ...
        let result: any = await this.MyRequest(options);
    ...
    async getAccessToken(_code: string): Promise<AccessTokenData> {//silent login
        ...
        let result: any = await this.MyRequest(options, data);
        return result;
        // if (result == null) {
    ...
    async refreshAccessToken(_refreshToken: string): Promise<AccessTokenData> {
    ...
    async createOrder(_orderID: string, _productName: string, _amount: number): Promise<TradeOrderID> {
    ...
    async MyRequest(_options: any, _data: any = null) {
